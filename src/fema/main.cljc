(ns fema.main
  "Kotodama WASM entrypoint for the Fema clean-room actor (L4) — Clojure port.
  ...
  facts (`fema.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"fema.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fema-compat" :tier tier :entities entities} 200])
